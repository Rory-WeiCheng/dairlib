...
std::pair <LCS, double> LCSFactoryFrankaRef::LinearizePlantToLCS(
  ...
  /// This new lcs factory take in not-sorted contact_geoms (named contact_geoms_orig) and do sorted pair operations in
  /// the function to provide python bindings for collecting data
  /// transplant the part the originally is in c3_controller.cc
  /// only works for c3 ball-rolling task
